...
mcmc_mint_object <- readRDS(here("minor_revision", "manuscript",
                                 "run_models", 
                                 "mcmc_mint_object_0405_random.rds"))
```
...
```{r compute-er-mint-data-part-ordinal, echo = FALSE,  results = "hide"}
mcmc_mint_ordinal <- 
  readRDS(here("minor_revision", "manuscript", "run_models", 
               "mcmc_mint_object_ordinal_0805.rds"))
```
...
mcmc_mint_ordinal$samples <- mcmc_mint_ordinal$samples[sample(1:1600000, 100000),]
```
...
To better decide on which proposals should be funded in each section individually, and where, if needed, a random selection group should be defined, consider Figure \ref{fig:ER_dist_all_mint_ind}. Hence, looking at the section seperately would lead to a random selection group of 8 in Section 1, which is `r round(8/87*100, 1)`% of all proposals evaluated in this section; 2 (`r round(2/8*100, 1)`%) of these can still be funded. No random selection group is needed for section 2, while three more proposals can still be funded among five in the random selection group in section 3, and three more proposals in a random selection group of seven for section 4. 
